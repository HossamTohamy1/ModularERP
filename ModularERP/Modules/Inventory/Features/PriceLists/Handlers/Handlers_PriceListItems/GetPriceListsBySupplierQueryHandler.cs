...
{
    public class GetPriceListsBySupplierQueryHandler : IRequestHandler<GetPriceListsBySupplierQuery, ResponseViewModel<IEnumerable<PriceListListDto>>>
    {
        ...
        public GetPriceListsBySupplierQueryHandler(
        ...
        public async Task<ResponseViewModel<IEnumerable<PriceListListDto>>> Handle(GetPriceListsBySupplierQuery request, CancellationToken cancellationToken)
        {
            try
            {
                var assignedPriceListIds = await _assignmentRepository
                    .GetAll()
                    .Where(x => x.EntityId == request.SupplierId &&
                               (x.EntityType == PriceListEntityType.Supplier ||
                                x.EntityType == PriceListEntityType.SupplierGroup))
                ...
                return ResponseViewModel<IEnumerable<PriceListListDto>>.Success(
                    priceLists,
                    $"Retrieved {priceLists.Count} price lists for supplier successfully"
                );
            }
            catch (Exception ex)
            {
                throw new BusinessLogicException(
                    $"Error retrieving supplier price lists: {ex.Message}",
                    "Inventory"
                );
            }
        }
