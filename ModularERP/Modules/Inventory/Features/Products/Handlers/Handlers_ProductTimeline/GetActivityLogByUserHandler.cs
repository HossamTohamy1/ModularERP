...
{
    public class GetActivityLogByUserHandler : IRequestHandler<GetActivityLogByUserQuery, ResponseViewModel<PaginatedResult<ActivityLogDto>>>
    {
        private readonly IGeneralRepository<ActivityLog> _repository;
        private readonly IMapper _mapper;

        public GetActivityLogByUserHandler(IGeneralRepository<ActivityLog> repository, IMapper mapper)
        {
        ...
        public async Task<ResponseViewModel<PaginatedResult<ActivityLogDto>>> Handle(
            GetActivityLogByUserQuery request, CancellationToken cancellationToken)
        {
            try
            {
                var query = _repository.Get(x => x.UserId == request.UserId)
                ...
                return ResponseViewModel<PaginatedResult<ActivityLogDto>>.Success(result, "User activity logs retrieved successfully");
            }
            catch (Exception ex)
            {
