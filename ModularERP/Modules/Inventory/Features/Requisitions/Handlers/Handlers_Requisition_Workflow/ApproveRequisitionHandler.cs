...
{
    public class ApproveRequisitionHandler : IRequestHandler<ApproveRequisitionCommand, ResponseViewModel<bool>>
    {
        ...
        public async Task<ResponseViewModel<bool>> Handle(ApproveRequisitionCommand request, CancellationToken cancellationToken)
        {
            ...
            if (requisition.Status != RequisitionStatus.Submitted)
            {
                throw new BusinessLogicException(
                    $"Only submitted requisitions can be approved. Current status: {requisition.Status}",
                    MODULE_NAME,
