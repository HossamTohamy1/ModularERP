...
{
    public class DeleteCustomFieldHandler
    {
        private readonly ICustomFieldService _service;
        private readonly IValidator<DeleteCustomFieldCommand> _validator;

        public DeleteCustomFieldHandler(
            ICustomFieldService service,
            IValidator<DeleteCustomFieldCommand> validator)
        {
            _service = service;
            _validator = validator;
        }

        public async Task<ResponseViewModel<bool>> Handle(DeleteCustomFieldCommand command)
        {
            ...
            if (!validationResult.IsValid)
            {
                ...
                throw new ModularERP.Common.Exceptions.ValidationException(
                    "Validation failed for custom field deletion",
                    errors,
                    "Inventory");
            }

            return await _service.DeleteAsync(command);
        }
    }
}
