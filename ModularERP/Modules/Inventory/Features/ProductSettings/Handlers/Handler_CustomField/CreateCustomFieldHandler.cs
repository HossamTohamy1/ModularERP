...
{
    public class CreateCustomFieldHandler
    {
        private readonly ICustomFieldService _service;
        private readonly IValidator<CreateCustomFieldCommand> _validator;

        public CreateCustomFieldHandler(
            ICustomFieldService service,
            IValidator<CreateCustomFieldCommand> validator)
        {
            _service = service;
            _validator = validator;
        }

        public async Task<ResponseViewModel<CustomFieldResponseDto>> Handle(CreateCustomFieldCommand command)
        {
            ...
            if (!validationResult.IsValid)
            {
                ...
                throw new ModularERP.Common.Exceptions.ValidationException(
                    "Validation failed for custom field creation",
                    errors,
                    "Inventory");
            }
