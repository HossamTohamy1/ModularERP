...
namespace ModularERP.Modules.Purchases.Payment.Validators.Validators_PaymentMethod
{
    public class UpdatePaymentMethodValidator : AbstractValidator<UpdatePaymentMethodCommand>
    {
        ...
        public UpdatePaymentMethodValidator(IGeneralRepository<PaymentMethod> repository)
        {
        ...
        private async Task<bool> BeUniqueName(UpdatePaymentMethodCommand command, string name, CancellationToken cancellationToken)
        {
            return !await _repository.AnyAsync(
                x => x.Name.ToLower() == name.ToLower() && x.Id != command.Id,
                cancellationToken);
        }

        private async Task<bool> BeUniqueCode(UpdatePaymentMethodCommand command, string code, CancellationToken cancellationToken)
        {
            return !await _repository.AnyAsync(
                x => x.Code.ToUpper() == code.ToUpper() && x.Id != command.Id,
                cancellationToken);
        }
    }
}
